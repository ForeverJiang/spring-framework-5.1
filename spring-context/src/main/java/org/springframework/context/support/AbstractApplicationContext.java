...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
    ...
    public void refresh() throws BeansException, IllegalStateException {
        synchronized (this.startupShutdownMonitor) {
            ...
            /**
             * 告诉子类刷新内部bean工厂
             * obtainFreshBeanFactory: 获取新的Bean工厂
             *
             * 这句以后代码的都是注册容器的信息源和生命周期事件，载入过程就是从这句代码启动。
             * AbstractApplicationContext 的 obtainFreshBeanFactory 方法
             ...
             * 和 refreshBeanFactory 方法类似，载入Bean定义的方法 loadBeanDefinitions 也使用了委派模式，
             * 在AbstractRefreshableApplicationContext类中只定义了抽象方法，具体的实现调用子类容器中的方法实现。
             *
             */
             ...
             *
             * prepareBeanFactory方法在AbstractApplicationContext.refresh流程中。
             * 对前面获取到的beanFactory(ConfigurationListableBeanFactory)进行相关的设置，
             * 包括ClassLoader, post-processors，为后续的使用做准备。
             * setBeanExpressionResolver 设置Bean表达式解析器
             */
    ...
    protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
         ...
         *
         * Spring容器的refresh方法内部调用prepareBeanFactory方法，
         * prepareBeanFactory方法会添加ApplicationContextAwareProcessor到BeanFactory中。
         * 这个Processor的作用在于 为实现*Aware接口的bean调用该Aware接口定义的方法，并传入对应的参数。
        ...
        /**
         * ignoreDependencyInterface。从官方文档的“字面”来看，其作用指定自动装配(autowiring)的时候忽略的接口。
         * 还有一个很相似的方法叫ignoreDependencyType，同样其官方字面意思是指自动装配(autowiring)的时候忽略的类。
         * 结论：
         ...
         * }
         *
         * 这样的做法使得ApplicationContextAware中的ApplicationContext依赖在自动装配时被忽略，
         * 而统一由框架设置依赖，如ApplicationContextAware接口的设置会在ApplicationContextAwareProcessor类中完成；
         *
         *
         ...
         * }
         *
         * 通过这种方式保证了ApplicationContextAware中的容器保证是生成该bean的容器。
         *
         */
    ...
    protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
        /**
         * getBeanFactoryPostProcessors() 它是获取的手动注册（自定义）的BeanFactoryPostProcessor。
         * 何为手动注册：即未加@Component的、自定义的、实现了BeanFactoryPostProcessor的类（即没有交给Spring管理）
         *
         * getBeanFactoryPostProcessors()函数返回了 AbstractApplicationContext 中保存的 beanFactoryPostProcessors
         */
